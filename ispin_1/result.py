...
################# Input the date
reference_level = -2.3305
kpath = ['X', r'$\Gamma$', 'Y', 'S', r'$\Gamma$']
y_axis_min = -3
y_axis_max = 3.01
...
def band_plot_up(bands, axis_x, axis_y_up, delline):
    ...
    for i in range(0, bands):
        plt.plot(axis_x, axis_y_up[i, :], color='black', linewidth=1.0, linestyle="-")
    # Plot dished lines
    wdish = len(delline)
    for i in delline:
        wdish = wdish - 1
    ...
    if len(delline) + 2 == len(kpath):
        ...
        plt.xticks(xlocs, kpath, size=32)


def procar_plt_up(ion, orb, mult, dotcolor):
    pro_axis_up, pro_data_up = get_plot_procar_axis_y_up(ion, orb, procar_axis_x, bands, compress_radio,
                                                         kpt_insert_point)
...
if __name__ == '__main__':
    temp_bands, temp_kpoints, temp_axis_up, temp_delline = read_EIGENVAL(reference_level,
                                                                         'EIGENVAL')  # Get temporary bands data
    cbm, vbm = get_cbm_vbm(temp_axis_up)  # Get the vbm and cbm
    start_band, end_band = get_band_range_up(y_axis_min, y_axis_max,
                                             reference_level)  # Get the required bands range from the drawing
    ...
    fig = plt.figure(figsize=(10.8, 8))  # Canvas initialization and set size
    ...
    i = 3
    procar_plt_up(i, dict1['px'], 500, 'red')
    procar_plt_up(i, dict1['py'], 500, 'skyblue')
    procar_plt_up(i, dict1['pz'], 500, 'seagreen')
    # Set the range of abscissa and ordinate
    plt.xlim(min(axis_x), max(axis_x))
    plt.ylim(y_axis_min, y_axis_max)
    plt.yticks(np.arange(y_axis_min, y_axis_max, 1.0), size=25)
    # Set the xlabel and xylabel  and title
    plt.ylabel('E-$\mathregular{E_{Fermi}}$(eV)', size=35)

    # Set the legend
    ax.scatter(10000, 10000, s=240, c='red', edgecolor='none', alpha=1.0, label='Te-px')
    ax.scatter(10000, 10000, s=240, c='skyblue', edgecolor='none', alpha=1.0, label='Te-py')
    ax.scatter(10000, 10000, s=240, c='seagreen', edgecolor='none', alpha=1.0, label='Te-pz')
    font1 = {'weight': 'normal', 'size': 20}
    legend = plt.legend(prop=font1, loc='upper right', scatterpoints=3)
